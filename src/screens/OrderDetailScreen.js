...
export default function OrderDetailScreen() {
  ...
  const [order] = useState({
    _id: orderId, orderNo:'SF20260629120001',
    items:[{name:'明星联名T恤',qty:1,price:9900}],
    totalAmount:9900, status:'paid', paidAt: new Date().toISOString(), createdAt: new Date(Date.now()-3600000).toISOString(),
    ...
    <View style={styles.page}>
      <ScrollView>
        <View style={[styles.statusCard,{backgroundColor:cfg.bg}]}>
          ...
          <Text style={{fontSize:18,fontWeight:'700',color:colors.text,marginTop:8}}>{cfg.text}</Text>
        ...
        {order.express && (
          <View style={styles.card}>
            ...
            <View style={{flexDirection:'row',justifyContent:'space-between',marginTop:8}}>
              <Text style={{fontSize:13}}>{order.express.company}</Text>
              <Text style={{fontSize:13,color:colors.hpPink}}>{order.express.no}</Text>
            </View>
          </View>
        )}
        ...
        <View style={styles.card}>
          ...
          <View style={styles.row}><Text style={styles.rLabel}>订单编号</Text><Text style={{fontSize:13,color:colors.hpPink}}>{order._id}</Text></View>
          <View style={styles.row}><Text style={styles.rLabel}>下单时间</Text><Text style={{fontSize:13}}>{formatTime(order.createdAt)}</Text></View>
          {order.paidAt && <View style={styles.row}><Text style={styles.rLabel}>付款时间</Text><Text style={{fontSize:13}}>{formatTime(order.paidAt)}</Text></View>}
          <View style={[styles.row,{borderBottomWidth:0}]}><Text style={styles.rLabel}>订单金额</Text><Text style={{fontSize:16,fontWeight:'700',color:colors.price}}>{formatPrice(order.totalAmount)}</Text></View>
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  rLabel: { fontSize:13,color:colors.textSecondary },
