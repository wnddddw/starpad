      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 100 + insets.bottom }}>
        ...
        <View style={styles.plansRow}>
          {PLANS.map(p => {
            ...
            return (
              <TouchableOpacity key={p.key} style={[styles.plan, isSel && styles.planSel, p.popular && { borderColor: colors.gold }]}
                onPress={() => setSelected(p.key)}>
                {p.popular && <View style={styles.popBadge}><Text style={{ fontSize: 9, color: '#9d5b20', fontWeight: '600' }}>最受欢迎</Text></View>}
                <Text style={styles.planName}>{p.name}</Text>
                <Text style={{ fontSize: 11, color: colors.textMuted, marginTop: 2 }}>{p.period}</Text>
                <Text style={styles.planPrice}>¥{p.price / 100}</Text>
                {p.benefits.map((b, i) => <Text key={i} style={{ fontSize: 10, color: colors.textSecondary, marginTop: 4 }}>✓ {b}</Text>)}
                {isSel && <Text style={{ fontSize: 11, color: colors.hpPink, fontWeight: '700', marginTop: 8 }}>✓ 已选择</Text>}
              </TouchableOpacity>
            );
          })}
        </View>
        ...
        <View style={styles.autoRow}>
          ...
          <Switch value={autoRenew} onValueChange={setAutoRenew} trackColor={{ false: colors.border, true: colors.hpPink }} />
        </View>
      </ScrollView>

      <View style={[styles.payBar, { paddingBottom: 16 + insets.bottom }]}>
        <View>
          <Text style={{ fontSize: 11, color: colors.textMuted }}>合计</Text>
          <Text style={{ fontSize: 20, fontWeight: '800', color: colors.text }}>{formatPrice(PLANS.find(p => p.key === selected)?.price || 0)}</Text>
        </View>
